...
## Recent changes
- 2026-06-29 — Final FR pass — the entire human-facing surface is now French. Translated the trial + behavioral nurture emails (`services/email.js`, fr-FR dates, prices `299 €/mois` etc.), the Stripe success page (`routes/checkout.js`, `lang="fr"`), the auth API error messages shown in the login/signup forms (`routes/auth.js`: "Identifiants invalides", "E-mail et mot de passe requis"…), and the 500 handler (`server.js`: "Une erreur est survenue."). Internal-only API JSON (`routes/trial-emails.js`), console logs and code comments stay English by design.
- 2026-06-29 — Brand wordmark restyled **Bildup → BildUp** (camelCase emphasises "Up" = the build-up / value-creation thesis). Applied case-sensitively to all *display* text across landing, auth, product, briefs, emails, checkout page, README and `public/llms.txt`. Lowercase identifiers intentionally kept: domain `bildup.com`, email `hello@bildup.com`, asset paths `/brand/bildup-mark.svg`, social handle `@bildup`. Logo/monogram assets unchanged.
- 2026-06-29 — Product workspace (`views/app/`) translated to French — the whole app is now FR. All authenticated pages (dashboard, onboarding, targets, company, alerts, pipeline, settings, not-found) + app-nav/app-head. Enum **keys** stay English (they drive CSS modifiers + DB values); French **display** labels centralised in `lib/format.js` (`country/availability/ownership/alertType/severity/band/stage`), with `eur/num/timeAgo` switched to fr-FR. `services/scoring.js` component labels/notes, `services/briefs.js` (deterministic template + OpenAI prompt now FR), `jobs/signal-monitor.js` signal copy, and `services/onboarding.js` pipeline next-steps all translated. Seed data (sectors, names, signal copy) was already FR. (Emails translated the same day — see top entry.)
- 2026-06-29 — Landing + auth pages translated to French (France-first audience). All public copy now FR: navbar/hero/problem/agent/features/valuecreation/pricing/outcomes/closing/footer + login/signup. `<html lang="fr">`, og:locale `fr_FR`, FR meta/keywords/JSON-LD (Organization + SoftwareApplication + FAQPage). Pricing offers kept (Searcher 299 €/mo, Sponsor 999 €/mo, Firm dès 1 800 €/mo) with FR number formatting and toggle labels (Annuel/Mensuel). Brand wordmark "Bildup" at the time (restyled to "BildUp" the same day — see top entry).
